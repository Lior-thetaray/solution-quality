...
def config():
    ...
    Anomalous behavior indicating money laundering activity has been identified for customer-{{ activity.customer_name }} by a machine learning model with a probability of {{ algo.score | round(4)}}.
    
    The features with high rating, impacting the prediction are -
    {{ algo.features[0].display_name }} with a rating of {{ algo.features[0].rating }}-{{ algo.features[0].description }}
    {{ algo.features[1].display_name }} with a rating of {{ algo.features[1].rating }}-{{ algo.features[1].description }}
    {{ algo.features[2].display_name }} with a rating of {{ algo.features[2].rating }}-{{ algo.features[2].description }}
    ...
    Anomalous behavior indicating money laundering activity has been identified for customer - {{ activity.customer_id }} by a rule based
    model.
    Rule Details:
    ...
    All variable values and descriptions-
    ...
    - additional information associated with suspicious activity within up to 5 rows. Don't repeat information that is included in the features with high rating.
    The output should be limited to 10 rows and be formatted using markdown
    probability should be indicated in percentage with a maximum of 2 decimals.
    """
)])