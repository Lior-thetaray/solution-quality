"""
transactions_generator.py  — with amount_usd normalization

Adds:
  - amount_usd: transaction amount converted to USD using synthetic FX rates.
"""
...
STRUCTURING_AMOUNTS = [19990, 199900, 499900, 99990, 149900]  # near-threshold palette
...
# NEW: Synthetic FX rates to USD (tune as needed)
FX_RATES_TO_USD = {
    "UAH": 0.027,  # 1 UAH ≈ 0.027 USD (example)
    "USD": 1.00,
...
def pick_weighted(items, probs):
    return np.random.default_rng().choice(items, p=np.array(probs)/np.sum(probs))
...
def make_customers(
    ...
    for _ in range(n_customers):
        ...
        open_date = date.today() - timedelta(days=int(np.random.integers(365, 8*365)))
        ...
        website_hash = (f"WEB_{np.random.integers(1000,9999)}" if ctype in ("fop","legal_entity") and rand_bool(0.10) else None)
...
def _counterparty_sampler(anomalous: bool=False) -> Dict[str, object]:
    ...
    cp_name = f"CP_{np.random.integers(10000,99999)}" if rand_bool(0.5) else None
...
# -----------------------------
# Anomaly windows (monthly)
# -----------------------------
def select_anomalous_customers(customers: List[Customer], pct_anomalous_monthly: float, seed: Optional[int]) -> List[str]:
    if seed is not None:
        random.seed(seed + 7)
    n = max(1, int(len(customers) * pct_anomalous_monthly))
    return random.sample([c.customer_id for c in customers], k=n)
...
def generate_transactions(
    ...
    output_path: Optional[str] = None,   # ".csv" or ".parquet"
) -> pd.DataFrame:
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
    ...
    anomalous_cids = set(select_anomalous_customers(customers, pct_anomalous_monthly, seed))
    ...
    while cur_month <= end:
        ...
        for c in customers:
            ...
            monthly_n = int(np.random.integers(mmin, mmax+1))
            ...
            ts_offsets = np.sort(np.random.integers(0, span_seconds+1, size=monthly_n))
            ...
            for t in txn_times:
                ...
                # Choose currency and compute USD normalization (NEW)
                ...
                row = dict(
                    txn_id=rand_id("T", 12),
                    txn_ts=t.isoformat(timespec="seconds"),
                    customer_id=c.customer_id,
                    account_id=c.account_id,
                    # KYC snapshot
                    customer_type=c.customer_type,
                    date_of_birth=(c.dob.isoformat() if c.dob else None),
                    address_full=c.address_full,
                    occupation=c.occupation,
                    pep_indicator=c.pep_indicator,
                    industry_code=c.industry_code,
                    account_open_months=acc_age_m,
                    whitelist_flag=c.whitelist_flag,
                    # Txn
                    direction=direction,
                    method=method,
                    amount=round(float(amt), 2),
                    currency=currency,
                    amount_usd=amount_usd,  # NEW FIELD
                    purpose_code=purpose,
                    cross_border_flag=bool(cross_border_flag),
                    declared_counterparty_name=cp["declared_counterparty_name"],
                    balance_after_txn=round(new_bal, 2),
                    is_split_component=is_split_component,
                    # Counterparty
                    cp_id=cp["cp_id"],
                    cp_type=cp["cp_type"],
                    cp_jurisdiction=cp["cp_jurisdiction"],
                    cp_known_shell_flag=cp["cp_known_shell_flag"],
                    cp_is_bank_customer=cp["cp_is_bank_customer"],
                    # Endpoint
                    endpoint_type=ep["endpoint_type"],
                    ip_hash=ep["ip_hash"],
                    device_id_hash=ep["device_id_hash"],
                    geo_lat=ep["geo_lat"],
                    geo_lon=ep["geo_lon"],
                    atm_id=ep["atm_id"],
                    withdrawal_channel=ep["withdrawal_channel"],
                    pos_terminal_id=ep["pos_terminal_id"],
                    # Linkage breadcrumbs
                    rep_id_hash=shared_rep if (is_abnormal_month and rand_bool(0.2)) else c.rep_id_hash,
                    address_hash=c.address_hash,
                    website_hash=shared_site if (is_abnormal_month and style=="biz_split") else c.website_hash,
...
if __name__ == "__main__":
    demo = generate_transactions(
        start_date="2025-01-01",
        end_date="2025-03-31",
        n_customers=250,
        pct_anomalous_monthly=0.04,
        avg_txn_per_customer_per_month=(10, 28),
        seed=1337,
        output_path="synthetic_transactions.csv",
    )
    print(demo.head(5).to_string(index=False))
    print(f"\nRows: {len(demo):,} | Customers: {demo['customer_id'].nunique()} | Anomalous txns: {demo['anomaly_flag'].mean():.2%}")
