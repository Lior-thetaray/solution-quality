...
class SumRapidMovement(AggFeature, FeatureDescriptor):
    """
    required_params:
    ...
    def pre_aggs(self, params: dict) -> typing.OrderedDict[str, Column]:
        ...
        columns["sum_in_trx_3d"] = f.sum(f.when(f.col('direction') == 'credit', f.col('amount_usd'))).over(nday_window)
        columns["sum_out_trx_3d"] = f.sum(f.when(f.col('direction') == 'debit', f.col('amount_usd'))).over(nday_window)
        columns['sum_trx_ratio_3d'] = f.round(columns['sum_in_trx_3d'] / columns['sum_out_trx_3dn_day_trx_value_out'], 2)
        return columns

    def get_agg_exprs(self, params: dict) -> typing.OrderedDict[str, Column]:
        columns = OrderedDict()
        columns['sum_rapid_movement'] = f.when((columns['sum_trx_ratio_3d'] >= 0.9) & (columns['sum_trx_ratio_3d'] <= 1.1), 
                                             f.greatest(columns['sum_in_trx_3d'], columns['sum_out_trx_3d'])).otherwise(0)
        return columns

    @property
    def output_fields(self) -> Set[Field]:
        return {Field(identifier='sum_rapid_movement',
                      ...
                      dynamic_description=
                      """
                      Customer **{{ activity.customer_id }}** received transactions totaling **${{ activity.sum_in_trx | round(2) }}** and sent transactions totaling **${{ activity.sum_out_trx | round(2) }}** in a period of 3 days.
