...
class SumNameMis(AggFeature, FeatureDescriptor):
    """
    required_params:
    ...
    def get_agg_exprs(self, params: dict) -> typing.OrderedDict[str, Column]:
        columns = OrderedDict()
        columns['sum_trx_name_mis'] = f.round(f.sum(f.when(f.col('name_mis_ind') == 1, f.col('amount_usd'))),2)
        columns['cnt_dstnct_name_mis'] = f.count_distinct(f.when(f.col('name_mis_ind') == 1, f.col('cp_id')))
        return columns

    @property
    def output_fields(self) -> Set[Field]:
        return {Field(identifier='sum_trx_name_mis',
                      display_name='Name Mismatch Value',
                      description='sum_trx_name_mis',
                      category='KYC',
                      data_type=DataType.DOUBLE,
                      dynamic_description=
                """
                Customer **{{ activity.customer_id }}** conducted a total amount of **${{ activity.sum_trx_name_mis | prettify_number }}** with counterparties that have a mismatch between declared name and actual name.
                """,
                explainabilities=[
                    Explainability(
                        ...
                        values=[
                            ExplainabilityValueProperties(
                                ...
                                dynamic_value="sum_trx_name_mis",
               ...
               Field(identifier='cnt_dstnct_name_mis',
                      display_name='cnt_dstnct_name_mis',
                      description='Distinct Name Mismatch',
                      category='KYC',
                      data_type=DataType.LONG
                #       dynamic_description=
                # """
                # Customer **{{ activity.customer_id }}** conducted transactions with **{{ activity.cnt_dstnct_name_mis | prettify_number }}** distinct counterparties that have a mismatch between declared name and actual name.
                # """,
                # explainabilities=[
                #     Explainability(
                #         identifier="hist_expl",
                ...
                #             ExplainabilityValueProperties(
                #                 key="zval",
                #                 name="Mis Count Dstnct",
                #                 dynamic_value="cnt_dstnct_name_mis",
                #                 type=ExplainabilityValueType.TREND
                #             )
                #         ]
                #     )
