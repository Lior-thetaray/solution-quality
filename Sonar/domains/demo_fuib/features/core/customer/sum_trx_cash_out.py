...
class SumTrxCashOut(AggFeature, FeatureDescriptor):

    @property
    def identifier(self) -> str:
        return 'sum_trx_cash_out'
    ...
    def trace_query(self, params: dict) -> str:
        return "method = 'cash_withdrawal'"

    def get_agg_exprs(self, params: dict) -> OrderedDict[str, Column]:
        columns = OrderedDict()
        # count each cash transaction as 1
        columns['sum_trx_cash_out'] = f.sum(f.when( (f.col('method')=='cash_withdrawal'), f.col('amount_usd')))
        return columns
    ...
    def output_fields(self) -> Set[Field]:
        return {
            Field(
                identifier='sum_trx_cash_out',
                display_name='Total Cash Withdrawal Activity',
                ...
                dynamic_description=
                      """
                      Customer **{{ activity.customer_id }}** conducted cash withdrawal transactions totaling **${{ activity.sum_trx_cash_out | round(2) }}** in the current month.
                      """,
                explainabilities=[
                    Explainability(
                        ...
                        values=[
                            ExplainabilityValueProperties(
                                ...
                                dynamic_value="sum_trx_cash_out",
